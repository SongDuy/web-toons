    ...
    <div className="bg-white z-50">
      <div className="hidden lg:block">
        ...
        {OpenoAnimation && (
          <div>
            <button
              className={`fixed top-[50%] right-[3%] z-50 ${!OpennavRVivew ? "animate-slideLeft" : "animate-slideRight"}`} // Thêm z-index cao
            ...
            <div
              className={`fixed top-0 h-screen right-[-2%] w-[9%] bg-white p-8 z-50 ${!OpennavRVivew ? "animate-slideLeft" : "animate-slideRight"}`} // Thêm z-index cao
            >
              ...
              <div className="w-full h-full mt-5 overflow-auto"> {/* Thêm overflow-auto */}
                    ...
                    {comic.comic?.slice()?.sort((a, b) => b.views - a.views)?.map((item, index) => {
                      return (
                        <div className="grid grid-row-5 gap-1" key={item.id}>
                          ...
                          <p className="truncate transition-all text-sm">{item.title}</p>
