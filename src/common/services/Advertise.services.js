...
const advertiseFireBase = {
  async getAll() {
    const docSnap = await getDocs(collection(fireStore, "advertise"));
    const  advertise = docSnap.docs.map((item) => {
  ...
  async getAllid(id) {
    const docRef = query(
      collection(fireStore, "advertise"),
      where("uid", "==", id),
    );
  ...
  async getbyid(id) {
    const docRef = doc(fireStore, 'advertise', id);
  ...
  async Add(data) {
   const docid= await addDoc(collection(fireStore, "advertise"), data);
   return docid.id
  },

  async update(data, id) {
    const updatebanner = doc(fireStore, "advertise", id);
    ...
    const storageRef = ref(storage, `cms_uploads/advertise/${id}/${name}`);
  ...
  async Delete(id) {
    await deleteDoc(doc(fireStore, "advertise", id));
    await deleteFolder(`cms_uploads/advertise/${id}`)

  },
 
};
export default advertiseFireBase;
