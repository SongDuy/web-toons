...
const AdminContact = () => {
  ...
  const handlePhotoChange1 = async () => {
    try {
      ...
      await  contactFireBase.Add({Note:valueNote ,createTime: new Date(Date.now())})
  ...
  const handleupdate = async ( id) => {
    try {
      ...
      setloading(false);
        ...
        await contactFireBase.update({Note:valueNote ,createTime: new Date(Date.now())}, id)
  ...
  const handledelete = async (id) => {
    try {
      ...
      if (result) {
        setloading(false);
        await contactFireBase.Delete(id);
        setrelay(!relay)
        setOpen(false);
        setid("")
