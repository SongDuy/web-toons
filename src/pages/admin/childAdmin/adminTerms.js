...
const AdminTerms = () => {
  ...
  useEffect(() => {
    const get = async () => {
      try {
        ...
        seterms(terms.success ? terms.terms : []);
        setloading(true);
      } catch (error) { 
        console.log(error)
      }
  ...
  const handlePhotoChange1 = async (e) => {
    try {
      ...
      if (file) {
      const a=await  termsFireBase.Add({ createTime: new Date(Date.now())})
       await termsFireBase.uploadToAdd(file, file.name,a);
      }
  ...
  const handleupdate = async (e, id) => {
    try {
      ...
      const file = e.target.files[0];
      if (file) {
        await termsFireBase.uploadToAdd(file, file.name, id)
      }
      setrelay(!relay)
