...
const CommunityPolicy = () => {
  ...
  useEffect(() => {
    const get = async () => {
        try {
          ...
          setAdvertise(advertise.success ? advertise.advertise : []);
          setloading(true);
        } catch (error) { 
          console.log(error)
        }
  ...
  const file = useMemo(
      () => ({
        url:Advertise.length>0?Advertise[0]?.File:"https://firebasestorage.googleapis.com/v0/b/webtoons-2ae20.appspot.com/o/cms_uploads%2Fcomic%2Fepisodes%2FIAsCigDN5La3LV2GtFOl0YnL9kG3%2Fp21CPp0sVnIMjq2zgX0o%2Fchap%2F9fNgdUbNDG7mV5bi7dkf%2Fnhasachmienphi-truyen-tranh-doremon.pdf?alt=media&token=7543361b-cddf-4e4f-9aaa-9bfe8a91ce14",
      }),
      [Advertise]
    );
